  ...
  <tbody>
    ...
    <?php foreach($top_crashers as $crash){ ?>
      ...
      <tr class="<?php echo ( ($row) % 2) == 0 ? 'even' : 'odd' ?>">
        <td><div id="url-to-sig<?php echo $row; ?>" class="tcburl-toggler tcburl-urlToggler">+</div> <a id="tcburl-url<?php echo $row ?>" class="tcburl-urlToggler" href="#"><?php out::H($crash->url) ?></a> <a  href="<?php echo $crash->url ?>" target="_new">link</a> </td>
