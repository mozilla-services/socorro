...
if ($crash['is_hang'] == true) { ?>
    ...
    <?php if ($crash['is_plugin'] == true) {?>
              <?php if ($linked) { ?><a href="<?= $crash['link'] ?>" class="plugin-btn"><?php } ?><img src="<?= url::site('/img/3rdparty/fatcow/brick16x16.png')?>" width="16" height="16" alt="Plugin Crash" class="plugin" /><?php if ($linked) { echo '</a>'; } ?>
     <?php } ?>
<?php }
?>
