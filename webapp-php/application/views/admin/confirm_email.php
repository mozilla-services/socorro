    ...
    <div class="admin">
      ...
      <?php if (isset($estimated_count)) { ?>
        <p>Sending this email <strong>will contact <?= $estimated_count ?></strong> 
           <?= $email_product?> <?= $email_versions ?> users who crashed on <?= $email_signature ?> 
           between <?= $email_start_date ?> and <?= $email_end_date ?></p>
        <div><p><strong>Subject:</strong><pre><?=$email_subject ?></pre></p>
        <p><strong>Email Body:</strong><pre><?= $email_body?></pre></p></div>
        ...
        <p>Note: The number of contacts above is an estimate. We won't email
           ...
           <?= $email_signature ?> crash. Malformed email addresses may also inflate the estimate.
