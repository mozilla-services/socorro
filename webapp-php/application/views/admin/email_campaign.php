<?php if (isset($campaign)) { ?>
  ...
  <p><?= out::h($campaign->author) ?> emailed <?= out::h($campaign->email_count)?>  users who crashed on
    ...
    <abbr class="start date" title="<?= out::($campaign->start_date) ?>"><?= date('n-j-Y g:ia', strtotime($campaign->start_date)) ?></abbr> and 
    <abbr class="end date"   title="<?= out::($campaign->end_date)   ?>"><?= date('n-j-Y g:ia', strtotime($campaign->end_date)) ?></abbr>.</p>

    <p>Email sent <abbr class="create date" title="<?= out::($campaign->date_created) ?>"><?= date('n-j-Y g:ia', strtotime($campaign->date_created)) ?></abbr></p>
