...
@click.option(
    ...
    help="the key in the telemetry bucket to save the schema to",
)
@click.pass_context
def upload(ctx, destination_key):
    """Uploads schema to GCS bucket for Telemetry
    ...
    By uploading it to GCS not only do we bet on GCS being more read-reliable
    that github.com's server but by being in GCS AND unavailable that means the
