$(function() {
  ...
  if (cookie_secure.data('session-cookie-secure')) {
    ...
    if (location.protocol === 'http:') {
      // bad
      addVerdict(cookie_secure, "You're using <b>HTTP</b> but SESSION_COOKIE_SECURE set to true!", 'bad');
    } else {
      // fine
      addVerdict(cookie_secure, "You're using <b>HTTPS</b> and SESSION_COOKIE_SECURE set to true.", 'good');
    }
  } else {
    // then you better on HTTP
    if (location.protocol === 'http:') {
      addVerdict(cookie_secure, "You're using <b>HTTP</b> right now which is expected.", 'good');
    } else {
      addVerdict(cookie_secure, "You're using <b>HTTPS</b> but SESSION_COOKIE_SECURE set to false!.", 'bad');
    }
  }
  $.getJSON(location.pathname, { 'test-cookie': true })
    .done(function(r) {
      if (r.cookie_value === cookie_secure.data('cookie-value')) {
        addVerdict(cookie_secure, 'Happily able to set a cookie and retrieving it again.', 'good');
      } else {
        addVerdict(cookie_secure, 'Unable to set a cookie and retrieving it again.', 'bad');
      }
    })
    .fail(function() {
  ...
  $.getJSON(location.pathname, { 'test-caching': true })
    .done(function(r) {
      if (r.cache_value === caching.data('cache-value')) {
        // seems to work
        addVerdict(caching, 'Setting and retrieving from cache seems to work.', 'good');
      } else {
        addVerdict(
          caching,
          'Unable to set a value in cache that sticks! ' + 'Check your <code>settings.CACHES</code> settings.',
          'bad'
        );
      }
    })
