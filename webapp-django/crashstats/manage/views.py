...
def field_lookup(request):
    ...
    if not name:
        return CrashstatsHttpResponseBadRequest("Missing 'name'")
...
def supersearch_field_create(request):
    ...
    if isinstance(field_data, basestring):
        return CrashstatsHttpResponseBadRequest(field_data)
...
def supersearch_field_delete(request):
    ...
    if not field_name:
        return CrashstatsHttpResponseBadRequest('A "name" is needed')
...
def api_tokens_delete(request):
    if not request.POST.get('id'):
        return CrashstatsHttpResponseBadRequest('No id')
