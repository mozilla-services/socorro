...
def test_highlight_url(client, requests_mock):
    ...
    requests_mock.get(f'https://{HOST}/200.h', text=dedent("""
        //
...
def test_highlight_line(client, requests_mock):
    ...
    with mock.patch('crashstats.sources.views.HtmlFormatter', wraps=HtmlFormatter) as MockFormatter:
        ...
        response = client.get(url, {'url': f'https://{HOST}/200.h', 'line': 'not_a_number'})
        assert response.status_code == 200
        MockFormatter.assert_called_with(title='/200.h', hl_lines=[], **static_kwargs)
        ...
        assert response.status_code == 200
        MockFormatter.assert_called_with(title='/200.h', hl_lines=[403], **static_kwargs)
