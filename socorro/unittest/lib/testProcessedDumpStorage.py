...
class TestDmpStorage(unittest.TestCase):
  ...
  def constructorAlt(self,*args,**kwargs):
    ...
    assert os.path.join(self.testDir,kwargs.get('rootName','.')) == storage.storageBranch
    assert storage.rootName == kwargs.get('rootName','.'),'From kwargs=%s'%kwargs
  ...
  def testPutDumpToFile(self):
    ...
    storage.putDumpToFile(ooid,data)
