...
a_raw_crash = {"submitted_timestamp": "2013-01-09T22:21:18.646733+00:00"}
...
def setup_mocked_s3_storage(cls=S3ConnectionContext, **extra):
    ...
    config = get_config(cls=cls, values_source=values_source)
...
class TestConnectionContext:
    ...
    def test_submit(self):
        ...
        conn.submit(
            "fff13cf0-5671-4496-ab89-47a922141114", "name_of_thing", thing_as_binary
        )
        ...
        with MetricsMock() as mm:
            ...
            assert (
                len(
                    mm.filter_records(
                        ...
                        tags=["kind:name_of_thing", "outcome:successful"],
                    )
                )
                == 1
            )
            assert (
                len(
                    mm.filter_records(
                        stat="processor.s3.submit",
                        tags=["kind:name_of_thing", "outcome:failed"],
                    )
                )
                == 1
            )
    ...
    def test_fetch(self):
        ...
        mocked_get_contents_as_string = (
            conn._connect_to_endpoint.return_value.get_bucket.return_value.get_key.return_value.get_contents_as_string
        )
        mocked_get_contents_as_string.side_effect = [thing_as_binary]

        # the tested call
        result = conn.fetch("fff13cf0-5671-4496-ab89-47a922141114", "name_of_thing")
        ...
        mocked_get_contents_as_string.assert_has_calls([mock.call()])
    ...
    def assert_regional_s3_connection_parameters(self, region, conn):
        ...
        args = (region,)
        conn._connect_to_endpoint.assert_called_with(*args, **kwargs)

    def test_fetch_with_regional_s3connection_context(self):
        # setup some internal behaviors and fake outs
        conn = setup_mocked_s3_storage(
            cls=RegionalS3ConnectionContext, region="us-south-3"
        )
        conn.fetch("fff13cf0-5671-4496-ab89-47a922141114", "name_of_thing")
        self.assert_regional_s3_connection_parameters("us-south-3", conn)
    ...
    def test_create_bucket_with_regional_s3connection_context(self):
        ...
        conn._get_or_create_bucket(conn._mocked_connection, "name-of-bucket")
        conn._mocked_connection.create_bucket.assert_called_with(
            "name-of-bucket", location="us-south-3"
        )

    def test_HostPortS3ConnectionContext_host_port_secure(self):
        # Test with secure=True
        conn = setup_mocked_s3_storage(
            cls=HostPortS3ConnectionContext, host="localhost", port=4569, secure=True
        )
        conn.fetch("fff13cf0-5671-4496-ab89-47a922141114", "name_of_thing")
        kwargs = {
            "aws_access_key_id": conn.config.access_key,
        ...
        conn = setup_mocked_s3_storage(
            cls=HostPortS3ConnectionContext, host="localhost", port=4569, secure=False
        )
        conn.fetch("fff13cf0-5671-4496-ab89-47a922141114", "name_of_thing")
        kwargs = {
            "aws_access_key_id": conn.config.access_key,
