...
from socorro.external.elasticsearch.query_service import Query
...
class IntegrationTestQuery(ElasticSearchTestCase):
    ...
    def get_config(self, more_overrides=None):
        ...
        config_manager = self.get_standard_config_manager(
            service_classes=Query,
            overrides=overrides,
        )
    ...
    def tearDown(self):
        ...
        config = self.get_config()
        self.storage.es.delete_index(config.services.Query.elasticsearch_index)
    ...
    #--------------------------------------------------------------------------
    @mock.patch('socorro.external.elasticsearch.query_service.pyelasticsearch')
    ...
    def test_get_with_indices(self, mocked_es):
        ...
        mocked_connection.search.reset_mock()
        index_schema = 'socorro_%Y%W'
        config = self.get_config(
            {
                'resource.elasticsearch.elasticsearch_index': index_schema
            }
        )
