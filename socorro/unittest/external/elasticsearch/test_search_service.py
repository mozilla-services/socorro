...
from socorro.external.elasticsearch.search_service import Search
...
class TestElasticSearchSearch(TestCase):
    ...
    def get_config(self, more_overrides=None):
        overrides = {
            'logger': mock.Mock(),
            'resource.elasticsearch.elasticSearchHostname': "localhost",
            ...
            'services.Util.cls': mock.Mock(),
        }
        if more_overrides:
            overrides.update(more_overrides)
        config_manager = ElasticSearchTestCase.get_standard_config_manager(
            service_classes=(Search, Util),
            overrides=overrides,
        )
    ...
    def test_get_counts(self):
        ...
        res = Search.get_counts(
            ...
            0,
            2,
            config.services.Search.platforms
        )
...
class IntegrationElasticsearchSearch(ElasticSearchTestCase):
    ...
    def setUp(self):
        ...
        self.config.services.Util.cls.return_value.get_versions_info \
            .return_value = {
                'EarlyOwl:11.0b1': {
    ...
    def tearDown(self):
        ...
        self.storage.es.delete_index(
            config.services.Search.elasticsearch_index
        )

    #--------------------------------------------------------------------------
    def test_search_single_filters(self):
    ...
    #--------------------------------------------------------------------------
    def test_search_plugin_terms(self):
        base_params = {
            'products': 'PluginSoft',
            'report_process': 'plugin',
        }
