...
class TestCrashes(TestCase):
    ...
    def get_instance(self, config=None):
        ...
        """
        self.test_config = config or self.get_dummy_config()
        return Crashes(config=self.test_config)

    #--------------------------------------------------------------------------
    def test_prepare_search_params(self):
        """Test Crashes.prepare_search_params()."""
        crashes = self.get_instance()
        print crashes.config
        print dict(crashes.config.services)
    ...
    def test_get_signatures_with_too_big_date_range(self):
        ...
        config.executor_identity = lambda: ''
        config['services.Crashes'] = DotDictWithAcquisition({
...
class IntegrationTestCrashes(PostgreSQLTestCase):
    ...
    def setUp(self):
        ...
        data. """
        super(IntegrationTestCrashes, self).setUp((Crashes, Util))
    ...
    def test_get_exploitibility_with_pagination(self):
        ...
        def insert_test_data(connection):
            ...
            execute_no_results(connection, insert)
        ...
        self.transaction(insert_test_data)
