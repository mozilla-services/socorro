...
#==============================================================================
class Search(CrashesReportSearchWebServiceBase):
    """
    Implement the /search service with PostgreSQL.
    """

    uri = r'/search/(signatures|crashes)/(.*)'
    ...
    def get(self, **kwargs):
        ...
        if params["terms"]:
            ...
            params["terms"] = Search.prepare_terms(
                params["terms"],
                params["search_mode"]
            )
        ...
        util_service = self.config.services.Util.cls(
            config=self.config.services.Util
        )
        params["versions_info"] = util_service.get_versions_info(**params)
        ...
        sql_count_query = " ".join((
            "/* socorro.external.postgresql.search_service.Search search"
            ".count */",
            "SELECT count(DISTINCT r.signature)",
            sql_from,
            sql_where
        ))
    ...
    def generate_sql_select(self, params):
        ...
        sql_select.append(
            "SUM (CASE WHEN r.process_type='plugin' THEN 1"
            "ELSE 0 END) as numplugin"
        )
        sql_select.append(
            "SUM (CASE WHEN r.process_type='content' THEN 1"
