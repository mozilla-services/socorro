...
DECLARE
    partition_name TEXT;
    vendorid TEXT := 'AdapterVendorID';
    deviceid TEXT := 'AdapterDeviceID';
...
partition_name := find_weekly_partition(updateday, 'signature_summary_graphics');
...
EXECUTE format(
    ...
    WITH graphics_info AS (
        SELECT
            ...
            , json_object_field_text(raw_crash, %L) as vendor_hex
            , json_object_field_text(raw_crash, %L) as adapter_hex
        ...
        WHERE
            raw_crashes.date_processed::date = %L
            AND reports_clean.date_processed::date = %L
    )
    SELECT
