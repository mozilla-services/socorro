...
#==============================================================================
class Util(PostgreSQLWebServiceBase):
    """
    Implement /util services with PostgreSQL.
    """

    uri = r'/util/(versions_info)/(.*)'

    #--------------------------------------------------------------------------
    def get_versions_info(self, **kwargs):
        """
        ...
        sql = """
    /* socorro.external.postgresql.util_service.Util.get_versions_info */
            WITH infos AS (
                SELECT
