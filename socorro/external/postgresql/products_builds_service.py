...
#==============================================================================
class ProductsBuilds(PostgreSQLWebServiceBase):

    """
    Implement the /products/builds service with PostgreSQL.
    """

    uri = r'/products/builds/(.*)'
    ...
    def post(self, **kwargs):
        ...
        # TODO:  look at refactoring this to a modern standard
        with self.get_connection() as connection:
            try:
                cursor = connection.cursor()

                # the following call does its own commit/rollback
            ...
            # no commit or rollback necessary
            except psycopg2.Error, e:
                error = str(e)
                self.config.logger.error(
                    "Failed inserting build data into PostgresSQL, reason: %s"
                    % error,
                    exc_info=True
                )
                if "CONTEXT" in error:
