...
class Search(ElasticSearchBase):
    ...
    def get(self, **kwargs):
        ...
        versions_service = self.config.services.Util.cls(
            config=self.config.services.Util
        )
        params["versions_info"] = versions_service.get_versions_info(**params)
        ...
        self.config.logger.debug(
            "Query the crashes or signatures: %s",
            json_query
        )
    ...
    def search_for_signatures(self, params, es_result, query):
        ...
        try:
            ...
            self.config.logger.error(
                "ElasticSearch returned something wrong: %s",
                es_result[0],
                exc_info=True
            )
            raise
