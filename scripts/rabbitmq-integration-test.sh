...
python socorro/external/postgresql/setupdb_app.py --database_name=breakpad --dropdb --force --createdb > setupdb.log 2>&1
if [ $? != 0 ]
then
...
python socorro/cron/crontabber_app.py --resource.postgresql.database_name=breakpad --job=weekly-reports-partitions --force >> setupdb.log 2>&1
if [ $? != 0 ]
then
...
python socorro/processor/processor_app.py --admin.conf=./config/processor.ini --resource.rabbitmq.host=$rmq_host --secrets.rabbitmq.rabbitmq_user=$rmq_user --secrets.rabbitmq.rabbitmq_password=$rmq_password --resource.rabbitmq.virtual_host=$rmq_virtual_host --resource.postgresql.database_name=breakpad  --new_crash_source.new_crash_source_class='socorro.external.rabbitmq.rmq_new_crash_source.RMQNewCrashSource' --processor.processor_class='socorro.processor.mozilla_processor_2015.MozillaProcessorAlgorithm2015' > processor.log 2>&1 &
sleep 1
python socorro/middleware/middleware_app.py --admin.conf=./config/middleware.ini --rabbitmq.host=$rmq_host --rabbitmq.rabbitmq_user=$rmq_user --rabbitmq.rabbitmq_password=$rmq_password --rabbitmq.virtual_host=$rmq_virtual_host --resource.postgresql.database_name=breakpad --web_server.wsgi_server_class=socorro.webapi.servers.CherryPy > middleware.log 2>&1 &
echo " Done."
