...
do
  ...
  VERSIONS=`psql -t -U $SECRETS_POSTGRESQL_DATABASE_USERNAME -h $RESOURCE_POSTGRESQL_DATABASE_HOSTNAME breakpad -c "select version_string from product_versions where product_name = '${I}' and '${SQL_DATE}' between build_date and sunset_date" | awk '{print $1}'`
    for J in $VERSIONS
    do
