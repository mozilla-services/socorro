...
class HBaseSource:
  def __init__(self, addr, row_range, max_rows = 2048, batch_size = 256):
  ...
  def items(self):
    ...
    while True:
      ...
      try:
        ...
        logger.debug('read %d rows of data from hbase ending at %s', nrows, curr_last_read_key)
...
class SourceWorker(TaskClass):
  ...
  def run(self):
    ...
    s3_path_tmpl = '{env}/v1/{ftype}/{uuid}'
    env = 'stage'
    ...
    for key, cols in self.source.items():
      ...
      self.queue.put((s3_path_tmpl.format(env = env,
                                          uuid = key[7:],
                                          ftype = 'dump_names'),
                      json.dumps(dump_names)))
...
class S3Worker(TaskClass):
  ...
  def write_to_s3(self, key, payload):
    k = Key(self.bucket)
    k.key = key
...
def main(num_workers = 64):
  ...
  # start s3 workers
  workers = [S3Worker('us-west-2', 'crashstats', queue, None)
  ...
  for i in xrange(0, len(key_ranges), num_hbase_workers):
    ...
    for j in range(len(krng)):
      src_workers.append(SourceWorker(queue, (thrift_hosts, krng[j])))
